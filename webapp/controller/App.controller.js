	...
	(Device, Controller, Filter, FilterOperator, JSONModel) => {
		...
		return Controller.extend("sap.ui.demo.todo.controller.App", {
			...
			addTodo() {
				...
				aTodos.push({
					title: oModel.getProperty("/newTodo/title"),
					dueDate: oModel.getProperty("/newTodo/dueDate"),
					completed: false,
				});

				oModel.setProperty("/todos", aTodos);
				oModel.setProperty("/newTodo", { title: "", dueDate: null });
			},
			...
			_applyListFilters() {
				...
				if (sI18nKey) {
					const oResourceBundle = this.getView()
						.getModel("i18n")
						.getResourceBundle();
					sFilterText = oResourceBundle.getText(sI18nKey, [
						this.sSearchQuery,
					]);
				}
				...
				this.getView()
					.getModel("view")
					.setProperty("/filterText", sFilterText);
			},
		});
	}
);
